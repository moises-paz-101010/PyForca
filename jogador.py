...
class GerenciadorJogador:
    ...
    arquivo_jogador = "dados/jogador.json"
    arquivo_modo_acesso = ["r","a","w"]
    encoding = "utf-8"
    jogadores = []
    dados_sobrescreveu = 0

    def carregar_jogadores(self):
        try:
            with open(self.arquivo_jogador, self.arquivo_modo_acesso[0], encoding=self.encoding) as arquivo:
    ...
    def salvar_jogadores(self):
        ...
        try:
            with open(self.arquivo_jogador, self.arquivo_modo_acesso[2], encoding=self.encoding) as arquivo:
                json.dump(conteudo, arquivo, indent=4)
        except:
            print(f"Algum error ocorreu ao tentar gravar no arquivo {self.arquivo_jogador}")